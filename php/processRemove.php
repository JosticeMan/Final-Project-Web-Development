	...
	<body onload = "initialize();">
		...
		<div style = "position: relative; top: 70px;">
			<?php
				...
				{
					displayMessage("You do not have this dino in your database! Please remove a different dinosaur!", "removeDino.php?Survivor=$survivor");
				}
				else
				{
					$cmd = "DELETE FROM `survivordinosaurs` WHERE `Survivor` = '$survivor' AND `Dino Type` = '$dinotype' AND `Dino Name` = '$dinoname' 
					AND `Dino Level` = '$dinolevel' AND `Dino Health` = '$dinohealth' AND `Dino Stamina` = '$dinostamina' AND
					`Dino Oxygen` = '$dinooxygen' AND `Dino Food` = '$dinofood' AND `Dino Melee Damage` = '$dinodamage' AND 
					`Dino Movement Speed` = '$dinospeed'";
					$result = $conn->prepare($cmd);
					$result->execute();
					displayMessage("Dinosaur successfully removed! Click on me to view your Dinosaurs!", "viewDino.php?Survivor=$survivor");
				}
